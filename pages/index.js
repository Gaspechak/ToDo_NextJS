...
const tableRowItem = (item, setIsOpen, setSelectedObj, mutate) => {
  return (
    <div key={item.id} className={`list-item ${item.done ? 'done' : ''}`} onClick={() => openDialog(item, setIsOpen, setSelectedObj)}>
      <input type="checkbox" checked={item.done} onClick={(e) => {
        e.stopPropagation()
      ...
      <svg className="remove-button" onClick={(e) => removeTodo(e, item.id, mutate)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
